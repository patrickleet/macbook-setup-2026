...
- Runtimes and package managers: Node.js, pnpm, Python, Rust, Go
- Direct binary tools via `http:`, including Upbound `up` and Crossplane CLI
- Infra tools: `kubectl`, `helm`, `kubefwd`, `kn`, `lima`, `colima`, `docker-cli`, `docker-compose`, `aws-cli`, `gh`, `op`, `stripe-cli`, `flyctl`, `jq`, `yq`, `sops`
